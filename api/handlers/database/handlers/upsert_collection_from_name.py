...
class UpsertCollectionFromName(Resource):
    def put(self):
        ...
        logging.info(f"Sending params: id:{args['name']}, qty:{args['qty']}")

        try:
            postgres = PostgresBase()
            postgres.upsert_collection_from_name(name=args['name'], qty=args['qty'])
            return '200'
        except:
            return '500'
